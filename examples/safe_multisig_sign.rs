...
use mixin_sdk_rs::safe_multisig::fetch_sign_safe_multisig_request;
...
async fn main() -> Result<(), Error> {
    ...
    let request = fetch_sign_safe_multisig_request(&args.id, &safe_user).await?;
    ...
    println!("raw_transaction: {}", request.raw_transaction.is_some());
    Ok(())
}
...
fn print_usage() {
    eprintln!(
        "usage: cargo run --example safe_multisig_sign -- --keystore <path> --id <request-id-or-hash>"
    );
}
